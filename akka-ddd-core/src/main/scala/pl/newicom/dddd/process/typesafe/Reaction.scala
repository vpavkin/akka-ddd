...
import pl.newicom.dddd.office.Contract
...
case class SendCommand[O, Cmd <: Command, OCmd <: Command, Evt <: DomainEvent, Err](officePath: OfficePath[O], command: Cmd)(implicit contract: Contract.Aux[O, OCmd, Evt, Err], ev: Cmd <:< OCmd) extends Reaction[Nothing]
