...
case class Deliver[O, +Cmd <: Command](officePath: OfficePath[O], command: Cmd)(implicit officeContract: OfficeContract[O] { type CommandImpl = Cmd }) extends EventReaction[Nothing]
